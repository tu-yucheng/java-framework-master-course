...
SimpleDataFormat只是其中的一个例子，我们可以将这些技术用于任何非线程安全对象。
...
**为了避免内存一致性问题，我们需要建立一个happens-before的关系**。这种关系只是保证一条特定语句的内存更新对另一条特定语句可见。
...
在上面的代码中，我们可以清楚地看到首先threadA获取lock1，threadB获取lock2。
